// working example from https://github.com/oliviertassinari/serviceworker-webpack-plugin/blob/master/docs/src/sw.js
const DEBUG = false;
...
if (DEBUG) console.log(`[SW] cache name: ${ CACHE_NAME }`);
...
self.addEventListener('install', event => {
	if (DEBUG) console.log('[SW] install event', event);
...
self.addEventListener('activate', event => {
	if (DEBUG) console.log('[SW] activate event', event);
	if (DEBUG) {
...
self.addEventListener('fetch', event => {
	if (DEBUG) console.log('[SW] fetch event', event);
...
self.addEventListener('push', event => {
	if( DEBUG )console.log('[SW] push event', event);
	const title = 'Notification title';
	const body = 'sample notification';
